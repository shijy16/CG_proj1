...
void Builder::buildScene2(Object** objs, int num) {
	scene = new Scene();
	for (int i = 0; i < num; i++) {
		scene->addObj(objs[i]);
	}
	...
	Sphere* lightSource = new Sphere(new Meterial(0, 0, 0, 0), new ColorTexture(Color(1, 1, 1)), Vector3(500 * SIZE, 800 * SIZE, 300* SIZE), 40.0);
		//new Area(new Meterial(0, 0, 0, 0), new ColorTexture(Color(1, 1, 1)), Vector3(190 * SIZE, 400 * SIZE, 480 * SIZE), Vector3(0, 50 * SIZE, 0), Vector3(50 * SIZE, 0, 0),50*SIZE);
	lightSource->setAsLight(new Light(Vector3(200 * SIZE, 800 * SIZE, 300 * SIZE), Color(1, 1, 1), 5000, 1));
	scene->addObj(lightSource);
	scene->addObj(w1);
	scene->addObj(w3);
}
...
void Builder::buildScene3(Object** objs, int num) {
	scene = new Scene();
	for (int i = 0; i < num; i++) {
		scene->addObj(objs[i]);
	}
	...
	//Sphere* lightSource = new Sphere(new Meterial(0, 0, 0, 0), new ColorTexture(Color(1, 1, 1)), Vector3(200 * SIZE, 200 * SIZE, 500 * SIZE), 40.0);
	//lightSource->setAsLight(new Light(Vector3(200 * SIZE, 200 * SIZE, 500 * SIZE), Color(1, 1, 1), 5000, 1));
	Area* lightSource = new Area(new Meterial(0, 0, 0, 0), new ColorTexture(Color(1, 1, 1)), Vector3(0 * SIZE, 0 * SIZE, 500 * SIZE), Vector3(0, 50 * SIZE, 0), Vector3(50 * SIZE, 0, 0),50*SIZE);
	lightSource->setAsLight(new Light(Vector3(0 * SIZE, 0 * SIZE, 500 * SIZE), Color(1, 1, 1), 5000, 1));
	scene->addObj(lightSource);
	//scene->addObj(w1);
	scene->addObj(w3);
}
...
cv::Mat Builder::rayTracer_begin() {
	RayTracer tracer = RayTracer(camera, scene,camera->lenX, camera->lenY,SIZE);
	tracer.run();
